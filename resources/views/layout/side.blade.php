...
<div id="side-container"
    ...
    <div
        class="transition-all duration-400 w-full h-[70%] flex flex-col justify-start items-center gap-6 p-3 overflow-y-auto overscroll-none">
        @if (Auth::user()->role == 'Super admin')
            <a href="{{ route('admin.index') }}"
                ...
                <span class="side-link-text">Admin</span>
            </a>
        @endif
        <a href="{{ route('home') }}"
            class="transition-all duration-400 w-full min-h-10 border-b-2 border-white flex justify-start items-center gap-3 font-bold px-2 hover:text-black hover:bg-white/90 active:scale-90">
    ...
    <div class="transition-all duration-400 w-full h-[10%] flex flex-col justfy-end items-center gap-3">
        <p id="side-footer-text"
            class="transition-all duration-400 w-full h-full flex flex-wrap justify-center items-center">
            Copyright &copy;<span class="transition-all duration-300 font-bold mx-2">InvTrack</span>
            {{ date('Y') }}
            &nbsp; tout droit reservé
        </p>
